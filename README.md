...
Corrects mini icons for Bodygear and Staff on the equipping screen.
...
Press **Ctrl+`+`/`-`** to increase or decrease zoom speed.
...
1. BepInEx is a modding framework needed as a link between the game and CRMod. Download **[BepInEx 5.4.23.5](https://github.com/BepInEx/BepInEx/releases/tag/v5.4.23.5/)** (the zip under Assets) and extract it into your Chaos Reborn folder. This is the folder where Steam or another game manager installed the game, e.g. `C:\SteamLibrary\steamapps\common\ChaosReborn`.
...
2. Download **`CRMod-v1.12.1.zip`** from the [Releases page](https://github.com/wwwweb/CRMod/releases) and extract it into the same folder. The files will drop into the right places automatically
...
4. Copy the Webhook URL and set it in the config file `BepInEx\config\CRMod.ChatCommand.cfg` at the according format
   ```ini
